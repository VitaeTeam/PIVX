...
class PIVX_RPCSporkTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert(not active["SPORK_7_FUNDAMENTALNODE_PAYMENT_ENFORCEMENT"])
        # activate SPORK 8
        new_value = 1563253447
        res = self.nodes[0].spork("SPORK_7_FUNDAMENTALNODE_PAYMENT_ENFORCEMENT", new_value)
        assert(res == "success")
        sleep(1)
        self.sync_all()
        sporks = self.nodes[1].spork("show")
        self.printDict(sporks)
        assert(sporks["SPORK_7_FUNDAMENTALNODE_PAYMENT_ENFORCEMENT"] == new_value)
        active = self.nodes[0].spork("active")
        assert (active["SPORK_7_FUNDAMENTALNODE_PAYMENT_ENFORCEMENT"])
